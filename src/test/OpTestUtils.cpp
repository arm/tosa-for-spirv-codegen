...
static std::string CreateEffceeCheckFromDataType(DataType dataType)
{
    switch (dataType)
    {
        ...
        case DataType::int4_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeInt 4 0)";
        case DataType::int8_t:
        case DataType::uint8_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeInt 8 0)";
        case DataType::int16_t:
        case DataType::uint16_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeInt 16 0)";
        case DataType::int32_t:
        case DataType::uint32_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeInt 32 0)";
        case DataType::float16_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeFloat 16)";
        case DataType::float32_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeFloat 32)";
        case DataType::bfloat16_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeFloat 16 BFLOAT16)";
        case DataType::bool_t:
            return R"(CHECK: [[DATATYPE:%\w+]] = OpTypeBool)";
        default:
            throw std::invalid_argument("Data type not recognised!");
    }
}
...
static std::string CreateEffceeCheckFromDataTypeDAG(DataType dataType)
{
    switch (dataType)
    {
        // Note types are all signless in TOSA SPIRV so for example int8_t will always be uint8_t i.e. signedness = 0.
        case DataType::int4_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeInt 4 0)";
        case DataType::int8_t:
        case DataType::uint8_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeInt 8 0)";
        case DataType::int16_t:
        case DataType::uint16_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeInt 16 0)";
        case DataType::int32_t:
        case DataType::uint32_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeInt 32 0)";
        case DataType::float16_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeFloat 16)";
        case DataType::float32_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeFloat 32)";
        case DataType::bfloat16_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeFloat 16 BFLOAT16)";
        case DataType::bool_t:
            return R"(CHECK-DAG: [[DATATYPE:%\w+]] = OpTypeBool)";
        default:
            throw std::invalid_argument("Data type not recognised!");
    }
}
                      ...
                      const std::string& goldenSPIRV)
{
    const std::string check = R"(
        ...
        )" + CreateEffceeCheckFromDataTypeDAG(dataType) + R"(
        CHECK-DAG: [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[INPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
        CHECK:      [[INPUT:%\w+]] = OpGraphInputARM [[INPUT_TYPE]]
        CHECK:                       )" + instruction + R"({{.*}}[[INPUT]]
    )";
                      ...
                      const std::string& goldenSPIRV)
{
    const std::string check = R"(
        CHECK:      [[LENGTH:%\w+]] = OpConstant {{%\w+}} )" + std::to_string(values.size()) + R"(
        CHECK-DAG:      [[ARRAY:%\w+]] = OpTypeArray {{%\w+}} [[LENGTH]]
        )" + CreateEffceeCheckFromDataType(dataType) + R"(
        CHECK-DAG: [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[INPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
        CHECK:      [[INPUT:%\w+]] = OpGraphInputARM [[INPUT_TYPE]]
        CHECK:                       )" + instruction + R"({{.*}}[[INPUT]]
...
{
    const std::string check = R"(
        CHECK-DAG:      [[LENGTH:%\w+]] = OpConstant {{%\w+}} )" + std::to_string(values.size()) + R"(
        CHECK-DAG:      [[ARRAY:%\w+]] = OpTypeArray {{%\w+}} [[LENGTH]]
        )" + CreateEffceeCheckFromDataTypeDAG(dataType) + R"(
        CHECK-DAG: [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[INPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
                      ...
                      const std::string& goldenSPIRV)
{
    ...
    EXPECT_TRUE(resultDAG.message().empty() ||
                resultNonDAG.message().empty() ||
                resultDTypeFirst.message().empty() ||
...
{
    const std::string check = R"(
        CHECK:      [[LENGTH_SHAPE_ARRAY:%\w+]] = OpConstant %uint )" + std::to_string(values.size()) + R"(
        )" + CreateEffceeCheckFromDataType(dataType) + R"(
        CHECK:       [[ARRAY:%\w+]] = OpTypeArray %uint [[LENGTH_SHAPE_ARRAY]]
        CHECK:  [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[OUTPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
        CHECK:        [[OUTPUT:%\w+]] = OpExtInst [[OUTPUT_TYPE]] {{%\w+}} )" + instruction + GetLastInstructionIDs(goldenSPIRV, instruction) + R"(
                                 ...
                                 const std::string& goldenSPIRV)
{
    const std::string check = CreateEffceeCheckFromDataType(dataType) + R"(
        ...
        CHECK:  [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[OUTPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
        CHECK:        [[OUTPUT:%\w+]] = OpExtInst [[OUTPUT_TYPE]] {{%\w+}} )" + instruction + R"(
...
{
    const std::string check = R"(
        CHECK-DAG:      [[LENGTH_SHAPE_ARRAY:%\w+]] = OpConstant %uint )" + std::to_string(values.size()) + R"(
        )" + CreateEffceeCheckFromDataTypeDAG(dataType) + R"(
        CHECK:       [[ARRAY:%\w+]] = OpTypeArray %uint [[LENGTH_SHAPE_ARRAY]]
        CHECK:  [[CONST_COMP:%\w+]] = OpConstantComposite [[ARRAY]] )" + ExpandValues(values) + R"(
        CHECK: [[OUTPUT_TYPE:%\w+]] = OpTypeTensorARM [[DATATYPE]] {{%\w+}} [[CONST_COMP]]
                      ...
                      const std::string& goldenSPIRV)
{
    ...
    EXPECT_TRUE(resultDataTypeFirst.message().empty() ||
                resultNonDAG.message().empty() ||
                resultTypeStruct.message().empty());
}
                              ...
                              const int index)
{
    ...
    {
        check = R"(
        CHECK: )" + args[index] + R"( = OpConstantCompositeReplicateEXT{{.*}})" + ExpandValues({values[0]}) + R"(
        )";
    }
    else
    {
        check = R"(
        CHECK: )" + args[index] + R"( = OpConstantComposite{{.*}})" + ExpandValues(values) + R"(
        )";
    }
                        ...
                        const std::string& goldenSPIRV)
{
    const std::string check = CreateEffceeCheckFromDataType(dataType) + R"(
        ...
        CHECK:                        )" + instruction + R"({{.*}}[[GRAPH_CONST]]
    )";
                   ...
                   const int index)
{
    ...
    const std::string check =
        CreateEffceeCheckFromDataType(dataType) + R"(
        CHECK: [[CONST:%\w+]] = OpConstant [[DATATYPE]] )" + std::to_string(value) + R"(
        CHECK:                  )" + instruction + R"( )" + AddSpacers(index) + R"([[CONST]]
    )";
                       ...
                       const int index)
{
    ...
    const std::string check =
        ...
        CHECK:                   )" + instruction + R"( )" + AddSpacers(index) + flagStr; //+ R"(
    //)";
