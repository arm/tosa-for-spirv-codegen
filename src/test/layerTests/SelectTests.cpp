...
TEST(TOSA2SPIRV_LAYERS, Select)
{
    ...
    auto input2 = graph.AddInput(Tensor(DataType::bool_t, std::vector<unsigned int>{1, 1, 1, 1}), 0);

    auto input3 = graph.AddInput(Tensor(DataType::bool_t, std::vector<unsigned int>{1, 1, 1, 1}), 0);

    auto output = Tensor(DataType::bool_t, std::vector<unsigned int>{1, 1, 1, 1});

    const auto res = graph.AddSelectOperator(input1, input2, input3, output);
