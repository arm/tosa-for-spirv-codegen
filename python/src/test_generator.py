...
class TestGenerator():
    def __init__(self,
        ...
        self.operator_exception = ['CUSTOM', 'CONST', 'IDENTITY', 'DIM', 'VARIABLE', 'VARIABLE_WRITE', 'VARIABLE_READ']
            ...
            if group.name not in unsupportedgroups:
                for op in group.operators:
                    if not op.name in self.operator_exception:
                        ...
                        operator_name_list.append(self.operator_name_to_pascal_case(op.name))
...
""".format(self.tosa_version)
    ...
    def get_tosa_attribute_dict(self):
        def def_value():
            return "[UNIMPLEMENTED ATTRIBUTE: Add to dictOpAttributes dictionary in frontend_generator.py]"
    ...
    def find_input_argument(self, op):
        for idx, arg in enumerate(op.arguments):
            ...
            isInType = self.has_argument_tensor_element_type_name(arg, 'in_t') or self.has_argument_tensor_element_type_name(arg, 'in_out_t')
    ...
    def operator_walk_arguments_by_categories_and_tensor_element_type(self, op, catnames, types):
        for arg in op.arguments:
            catindex = tosa_spirv.pick_arg_category_index(arg)
            if arg.categories[catindex].name in catnames and arg.tensor_element_type in types:
                yield arg
    ...
    def print_parse_resize(self, file):
        file.copy_from(self.input_template_dict["parser_resize_fix_input_path"], self.num_copyright_lines)

    def operator_walk_arguments_by_categories_and_not_tensor_element_type(self, op, catnames, types):
        for arg in op.arguments:
            catindex = tosa_spirv.pick_arg_category_index(arg)
            if arg.categories[catindex].name in catnames and arg.tensor_element_type not in types:
                yield arg

    def operator_walk_arguments_by_categories_and_not_shapes(self, op, catnames, shapes):
        for arg in op.arguments:
            catindex = tosa_spirv.pick_arg_category_index(arg)
    ...
    def has_array_attributes(self, op):
        for arg in op.arguments:
            if tosa_spirv.TOSAType.type_category_for_arg(arg) == tosa_spirv.TOSAType.ARRAY:
                return True
    ...
    def get_arg_type(self, op, arg):
        datatype = TOSA_TO_SPV_TYPE_DECL_NAME_MAP.get(op.typesupports[0].tymap.get(arg.tensor_element_type))
        if datatype == None:
    ...
    def has_tosa_type_array(self, arg):
        return tosa_spirv.TOSAType.type_category_for_arg(arg) == tosa_spirv.TOSAType.ARRAY
                ...
                if argument.name in op_fix_dict[op.name].keys():
                    for item in op_fix_dict[op.name][argument.name].keys():
                        if item == 'shape':
                            ...
                            argument.tensor_element_type = op_fix_dict[op.name][argument.name][item]
        return op
    ...
    def generate_attribute_arguments(self, op, argument, file):
        ...
        is_attr = False
        if int(numValues) > 1 or (op.name == 'RESCALE' and (argument.name == 'multiplier' or argument.name == 'shift')):
            ...
            if any(category.name == "attribute" for category in argument.categories) and tosa_spirv.TOSAType.type_category_for_arg(argument) != tosa_spirv.TOSAType.RANKED_TENSOR \
                ...
                is_attr = True
                if datatype == 'bool':
                    file.write('bool {} = {};\n'.format(argument.name, 'true' if values == '1' else 'false'))
                else:
                    file.write('std::vector<{}> {} = {{{}}};\n'.format(self.get_cpp_data_type(datatype),
                                                                       argument.name,
                                                                       values))
            # work around attribute incorrectly marked as input
            elif self.is_reshape_attribute(op, argument) or self.is_tile_attribute(op, argument) or self.is_transpose_attribute(op, argument):
                is_attr = True
                file.write('std::vector<int32_t> {} = {{{}}};\n'.format(argument.name, values))
        else:
            # single value attribute
            # fix resize mode attribute
            if argument.tensor_element_type == 'resize_mode_t':
                is_attr = True
                file.write('ResizeMode {} = ResizeMode_BILINEAR;\n'.format(argument.name))

            # fix PAD pad_const attribute, and CLAMP min_val/max_val attribute
            elif (op.name == "PAD" and argument.name == "pad_const") or op.name == 'CLAMP':
                is_attr = True
                file.write('int32_t {}_int = {};\n'.format(argument.name, values))
                file.write('float {}_fp = {};\n'.format(argument.name, values))
                attr_name = ['{}_int'.format(argument.name), '{}_fp'.format(argument.name)]

            # general attribute
            elif any(category.name == "attribute" for category in argument.categories) and tosa_spirv.TOSAType.type_category_for_arg(argument) != tosa_spirv.TOSAType.RANKED_TENSOR \
                    or self.check_fake_input(op, argument):
                is_attr = True
                if datatype == 'bool':
                    file.write('bool {} = {};\n'.format(argument.name, 'true' if values == '1' else 'false'))
            ...
            # work around attribute incorrectly marked as input
            elif self.is_reshape_attribute(op, argument) or self.is_transpose_attribute(op, argument):
                is_attr = True
                file.write('int32_t {} = {};\n'.format(argument.name, values))
    ...
    def get_arg_data_type(self, op, argument):
        ...
        datatype = TOSA_TO_SPV_TYPE_DECL_NAME_MAP.get(op.typesupports[0].tymap.get(argument.tensor_element_type))
    ...
    def generate_tensor_arguments(self, op, argument):
        ...
        tensor_line = 'auto* {}Tensor = new TosaSerializationTensor({}Name, {}Shape, {}, {{}});\n'.format(argument.name, argument.name,
                                                                                                          argument.name, self.get_dtype(datatype))
            ...
            if not ops[idx].name in self.operator_exception:
                if not self.force_generate:
                    if not self.check_layer_test(names[idx]):
                        ...
                        print('[Code Generator] Found existing {}.'.format(self.get_layer_test_path(names[idx])))
                        skip_layer_counter += 1
                else:
                    if not names[idx] in self.operator_exception:
                        self.generate_layer_test(ops[idx], names[idx])
                    else:
                        print('[Code Generator] Skipping {}'.format(self.get_layer_test_path(names[idx])))
                        skip_layer_counter += 1
                layer_counter += 1
        print('[Code Generator] Generated unit tests for {} layers, skipped {} layers'.format(layer_counter, skip_layer_counter))

    def generate_test_arguments(self, op, name, file):

        for argument in op.arguments:
            first_line = ''
            if any(category.name == "input" for category in argument.categories) and \
                    argument.shape != '[rank(shape)]' and argument.shape != '-' and argument.shape != '[2]' and argument.shape != '[4]' and argument.shape != '[NC]' and argument.shape != '[TABLE_SIZE]' and argument.shape != '[rank(shape1)]':
                first_line = 'auto {} = Tensor::CreateInput('.format(argument.name)
            elif any(category.name == "output" for category in argument.categories):
                first_line = 'auto {} = Tensor::CreateOutput('.format(argument.name)
            elif (any("attribute" in category.name for category in argument.categories) and tosa_spirv.TOSAType.type_category_for_arg(argument) == tosa_spirv.TOSAType.RANKED_TENSOR):
                first_line = 'auto {} = Tensor('.format(argument.name)


            else:
                first_line = 'auto {} = Tensor::CreateAttribute('.format(argument.name)
            if argument.shape == '[4]':
                numValues = '4'
                values = "1,1,1,1"
            elif argument.shape == '[3]':
            ...
            if any(category.name == "attribute" for category in argument.categories) and tosa_spirv.TOSAType.type_category_for_arg(argument) != tosa_spirv.TOSAType.RANKED_TENSOR:
                attributeArg = True
                if datatype == 'int32':
                    file.write('auto {}_val = Tensor::ConvertInt32tToUint32t({{{}}});\n'.format(argument.name, values))
                elif datatype == 'int16':
                    file.write('auto {}_val = Tensor::ConvertInt16tToUint32t({{{}}});\n'.format(argument.name, values))
                elif datatype == 'int8':
                    file.write('auto {}_val = Tensor::ConvertInt8tToUint32t({{{}}});\n'.format(argument.name, values))
                elif datatype == 'bool':
                    file.write('auto {}_val = Tensor::ConvertBoolToUint32t({{{}}});\n'.format(argument.name, values))
                elif datatype == 'float16':
                    file.write('auto {}_val = Tensor::ConvertFloat16ToUint32t({{{}}});\n'.format(argument.name, values))
                elif datatype == 'float32':
                    file.write('auto {}_val = Tensor::ConvertFloat32ToUint32t({{{}}});\n'.format(argument.name, values))
                # default to Int32
                else:
                    file.write('auto {}_val = Tensor::ConvertInt32tToUint32t({{{}}});\n'.format(argument.name, values))
            ...
            file.write(first_line + 'DataType::{}_t,\n'.format(datatype))


            file.write(' ' * num_indentation + 'std::vector<unsigned int>{ ')
            file.append(str(numValues));
            if attributeArg:
                file.append(' },\n')
                file.write(' ' * num_indentation + '{}_val'.format(argument.name))
                file.append(');\n')
            else:
                file.append(' },\n')
            if (any(category.name == "input" for category in argument.categories) or \
                any(category.name == "output" for category in argument.categories)) and \
                    argument.shape != '[rank(shape)]' and argument.shape != '-' and argument.shape != '[2]' and argument.shape != '[4]' and argument.shape != '[NC]' and argument.shape != '[TABLE_SIZE]' and argument.shape != '[rank(shape1)]':
                file.write(' ' * num_indentation + 'module,\n')
                file.write(' ' * num_indentation + '"{}");\n'.format(argument.name))

            if any(category.name == "input" for category in argument.categories) or \
                    any(category.name == "output" for category in argument.categories):
                if self.is_reshape_attribute(op, argument) or self.is_tile_attribute(op, argument)  or self.is_transpose_attribute(op, argument):
                    continue

                if any("attribute" in category.name for category in argument.categories) and tosa_spirv.TOSAType.type_category_for_arg(argument) == tosa_spirv.TOSAType.RANKED_TENSOR:
                    # file.write('// Add a graph constant and assign it to the created tensor\n')
                    pascalArgName = self.operator_name_to_pascal_case(argument.name)
                    pascalOpName = self.operator_name_to_pascal_case(op.name)
                    file.write(' ' * num_indentation + "graph->GetNextResId());\n")
                    file.write("module->AddGraphConstant(\"{}\", {});\n".format(argument.name, argument.name))
            file.append('\n')
        file.append('\n')

        for argument in op.arguments:
            if "input" in argument.name and argument.type == 'tensor_list_t':
                file.write("std::vector<graphbuilder::Tensor> inputs;\n")
    ...
    def get_effcee_string(self, op, is_graph=False):
        ...
        categoryEffceeCheckDict = {
            tosa_spirv.TOSAType.RANKED_TENSOR: 'spirvwriter::Check{INPUTOUTPUT}Tensor({{{VALUES}}}, DataType::{DATATYPE}, "{OP_NAME}", outputStr);',
            tosa_spirv.TOSAType.SHAPED_TENSOR: 'spirvwriter::Check{INPUTOUTPUT}Tensor({{{VALUES}}}, DataType::{DATATYPE}, "{OP_NAME}", outputStr);',
            tosa_spirv.TOSAType.SCALAR: 'spirvwriter::Check{BOOL}Constant(DataType::{DATATYPE}, "{OP_NAME}", outputStr, {VALUE}, {ARG_INDEX});',
            tosa_spirv.TOSAType.ARRAY: 'spirvwriter::CheckConstCompositeTensor({{{VALUES}}}, "{OP_NAME}", outputStr, {ARG_INDEX});',
            tosa_spirv.TOSAType.TENSOR_LIST: "UNKNOWNTENSOR_LIST",
            tosa_spirv.TOSAType.TENSOR_LIST_UNIFORM_ETYPE: 'spirvwriter::Check{INPUTOUTPUT}Tensor({{{VALUES}}}, DataType::{DATATYPE}, "{OP_NAME}", outputStr);',
            tosa_spirv.TOSAType.ENUM: 'spirvwriter::Check{BOOL}Constant(DataType::{DATATYPE}, "{OP_NAME}", outputStr, {VALUE}, {ARG_INDEX});',
        ...
        for argument in op.arguments:
            ...
            if is_graph:
                if op.name == 'AVG_POOL2D' and (argument.name == 'input_zp' or argument.name == 'output_zp'):
                    datatype = 'int8'
            ...
            if self.is_rescale_graph_constant(op, argument):
                effcee_check_str = '    spirvwriter::CheckGraphConstant({ 1 }, DataType::DATATYPE, "RESCALE", outputStr);\n'.replace("DATATYPE", str(datatype) + '_t')
                effceestring += effcee_check_str
                idx += 1
            # hack to account reshape having Arrays
            elif self.is_reshape_attribute(op, argument) or self.is_tile_attribute(op, argument)  or self.is_transpose_attribute(op, argument) or self.is_slice_attribute(op, argument):
                if self.has_argument_category_name(argument, 'input'):
                    index = 1
                else:
                    index = 0
                effcee_check_str = '    spirvwriter::CheckConstCompositeTensor({{{VALUES}}}, "{OP_NAME}", outputStr, {ARG_INDEX});\n'
                effceestring += effcee_check_str.format(VALUES=values, OP_NAME=op.name, ARG_INDEX=index)
                idx += 1
            # Handle scalars and enums
            elif tosa_spirv.TOSAType.type_category_for_arg(argument) == tosa_spirv.TOSAType.SCALAR or tosa_spirv.TOSAType.type_category_for_arg(argument) == tosa_spirv.TOSAType.ENUM:
                if self.has_argument_tensor_element_type_name(argument, 'bool_t') or datatype == 'bool':
                    datatype = 'bool'
                    effceestring += "    " + effcee_check_str.format(BOOL="Bool", OP_NAME=op.name, VALUE="true", ARG_INDEX=idx, DATATYPE="bool_t") + "\n"
                else:
                    datatype_str = str(datatype) + '_t'
                    effceestring += "    " + effcee_check_str.format(BOOL="", OP_NAME=op.name, VALUE="1", ARG_INDEX=idx, DATATYPE=datatype_str) + "\n"
                idx += 1
            # Handle Inputs
            # hack to account for mode attribute before inputs for resize
                ...
                datatype_str = str(datatype) + '_t'
                if (op.name == "TABLE" and inputIdx == 1) or (op.name == "PAD" and inputIdx > 0):
                    continue
                effceestring += effcee_check_str.format(OP_NAME=op.name, INPUTOUTPUT=functionName, VALUES=values, ARG_INDEX=inputIdx, DATATYPE=datatype_str) + "\n"
                inputIdx += 1
            # Handle outputs and non scalar/bool
                ...
                if "output" in argument.name and op.name == 'RESCALE' or op.name == 'FFT2D' or op.name == 'RFFT2D':
                    continue
                else:
                    idx += 1
    ...
    def generate_layer_test(self, op, name):
        ...
        print('[Test Generator] Generating {} ...'.format(self.get_layer_test_path(name)))
        ...
        file.write('#include <Module.hpp>\n')
        file.write('#include <OpTestUtils.hpp>\n')

        file.write('#include <gtest/gtest.h>\n')
        file.write('#include <TestUtils.hpp>\n\n')

        file.write('#include <iostream>\n\n')

        # namespaces
        file.write('using namespace tosa2spirv;\n')
        file.write('using namespace graphbuilder;\n\n')

        # test case
        file.write('// Automatically generated by frontend_generator.py\n')
        ...
        file.write('auto module = Module::Create();\n')
        file.write('auto graph = module->AddGraph();\n')
        file.append('\n')
        ...
        # layer class
        first_line = 'graph->Add{}Operator('.format(name, self.get_layer_variable_name(name))
        num_indentation = len(first_line)
        ...
        file.write("// Use Module to write the full binary.\n")
        file.write("auto binary = module->Write();\n")
        file.write("std::string outputStr(tosa2spirv::spirvwriter::DisassembleSPIRV(binary, true));\n\n")
        ...
        file.write("binary = module->Write();\n")
        file.write("outputStr = tosa2spirv::spirvwriter::DisassembleSPIRV(binary, true);\n\n")
        file.write(effceestring)
    ...
    def generate_CMake_lists_parser_test(self):
        ...
        operator_group_names = [self.operator_group_name_to_pascal_case(name) for name in operator_map.keys()]
    ...
    def generate_parser_tests(self):
        tosa_serialization_parser_test_path = self.tosa_serialization_parser_path + '/test/'
        ...
        for operator_group in operator_map.keys():
            ...
            file = FileWriter(tosa_serialization_parser_test_path + '{}OpsTest.cpp'.format(self.operator_group_name_to_pascal_case(operator_group)))
            file.write(self.preamble())
            file.copy_from(self.input_template_dict["parser_tests_path"], self.num_copyright_lines)
            # iterate through all operators in the operator groups
            for op in operator_map[operator_group]:
                print("[TosaSerializationParserTest Generator] Generating {} ...".format(op.name))
    ...
    def check_fake_input(self, op, argument):
        ...
        if op.name in fake_input_dict.keys() and argument.name in fake_input_dict[op.name]:
            return True
        return False
    ...
    def check_fake_attribute(self, op, argument):
        fake_attr_dict = {}
        if op.name in fake_attr_dict.keys() and argument.name in fake_attr_dict[op.name]:
            return True
        return False
    ...
    def generate_parser_test_case(self, op, file):
        ...
        for argument in op.arguments:
            ...
            elif any(category.name == "input" for category in argument.categories) and \
                argument.shape != '[rank(shape)]' and argument.shape != '-' and argument.shape != '[2]' and argument.shape != '[4]' and argument.shape != '[NC]' and argument.shape != '[TABLE_SIZE]' and argument.shape != '[rank(shape1)]':
                input_args.append(argument)
            elif any(category.name == "output" for category in argument.categories):
        ...
        if len(attr_list) > 0:
            ...
            # fix POOL attribute sequence
                ...
                attr_list[0], attr_list[1], attr_list[2] = attr_list[2], attr_list[0], attr_list[1]
        ...
        if len(attr_list) == 0:
            ...
            file.write(' ' * len(first_line) + "Attribute::Attribute_{},\n".format(self.dictOpAttributes[op.name]))
        ...
        file.write("// The raw pointers of operators and tensors will be deleted by the destructor of the block\n")
        ...
        file.write("const std::string outputStr(spirvwriter::DisassembleSPIRV(binarySpirv, true));\n")
        file.append("\n")
