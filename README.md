...
By default, the build will also produce the spirv2tosa library, `build/tools/spirv2tosa/libspirv2tosa.a`.
The library provides API calls allowing users to recreate TOSA Serialization Library structures from
a tosa2spirv `Module` instance, as well as generating C++ code usable for automatic unit test generation.

Along with the `spirv2tosa` library, a utility executable `build/tools/spirv2tosa/spirv2tosaTool` is also built.
The tool allows users to convert SPIR-V disassembly files into C++ TOSA Operator definitions and to generate
operator tests for the `tosa2spirv` library.
...
The file `TosaOperator.hpp` provides a graph-independent TOSA Operator representation not reliant on SPIR-V or the `tosa2spirv` library.
The file `SpirvParser.hpp` provides functions to convert `tosa2spirv::spirv::Instruction` instances into `TosaOperator.hpp`-compatible instances.
The file `spirv2tosa.hpp` provides the basic API allowing users to generate operator definitions or Tosa Serialization Library structures from a `tosa2spirv::spirv::Module`.
